...
public class IUserDaoImpl implements UserDao {
    ...
    public static void main(String[] args) throws Exception { // 做测试用
        ...
        user.setBirthday("2019-10-28");
        user.setSex("男");
        user.setTelephone("18612345678");
        user.setEmail("123456789@qq.com");
        user.setCode("123123123123123123123123123");
        user.setStatus("Y");
        String sql = "insert into tab_user(username,password,name,birthday,sex,telephone,email,status,code) values(?,?,?,?,?,?,?,?,?)";
        int count = new JdbcTemplate(JDBCUtils.getDataSource()).update(sql, user.getUsername(),
                user.getPassword(),
                user.getName(),
                user.getBirthday(),
                user.getSex(),
